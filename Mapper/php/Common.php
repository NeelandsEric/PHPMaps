...
//Connection Settings @0-0826BFB7
$CCConnectionSettings = array (
    "bluemonitor_Librarian" => array(
        "Type" => "PostgreSQL",
        "DBLib" => "PostgreSQL",
        "Database" => "bluemonitor",
        "Host" => "localhost",
        "Port" => "6432",
        "User" => "postgres",
        "Password" => "postgres",
...
//Initialize Common Variables @0-109F818F
...
CCConvertDataArrays();
$CCProjectStyle = "Pretentious-iulia";
...
//bluemonitor_Librarian Connection Class @-BB0D8A22
class clsDBbluemonitor_Librarian extends DB_Adapter
{
    function clsDBbluemonitor_Librarian()
    {
        $this->Initialize();
    }
    ...
    function Initialize()
    {
        ...
        $this->SetProvider($CCConnectionSettings["bluemonitor_Librarian"]);
        parent::Initialize();
...
//End bluemonitor_Librarian Connection Class
...
//CCGetUserID @0-2DF6AD86
function CCGetUserID()
{
    return CCGetSession("interface_user_id");
}
//End CCGetUserID

//CCGetGroupID @0-B7BB2510
function CCGetGroupID()
{
    return CCGetSession("interface_user_type");
}
//End CCGetGroupID

//CCGetUserLogin @0-8A0D579D
function CCGetUserLogin()
{
    return CCGetSession("ClickYourScada_nume");
}
//End CCGetUserLogin
...
//CCLoginUser @0-96696523
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBbluemonitor_Librarian();
    $SQL = "SELECT interface_user_id, interface_user_type, interface_user_password FROM interface_users WHERE interface_user_login=" . $db->ToSQL($login, ccsText) . " AND interface_user_password=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    $Result = $db->next_record();
    if ($Result) {
        CCSetSession("interface_user_id", $db->f("interface_user_id"));
        CCSetSession("ClickYourScada_nume", $login);
        CCSetSession("interface_user_type", $db->f("interface_user_type"));
...
//CCLogoutUser @0-F3E590FF
function CCLogoutUser()
{
    CCSetSession("interface_user_id", "");
    CCSetSession("ClickYourScada_nume", "");
    CCSetSession("interface_user_type", "");
